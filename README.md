...
Data quality failures are usually silent: the pipeline keeps running while nulls, zeros, and outliers quietly corrupt every dashboard downstream. Pipeline Doctor demonstrates the architecture production LLM systems need to be trustworthy — **deterministic statistical detection first, LLM reasoning second**. The LLM is never asked *whether* the data is bad (that's where hallucination is dangerous); it only explains *why* statistically confirmed anomalies happened and generates executable pandas fixes. Fast and testable where correctness is non-negotiable, intelligent where explanation adds value.

**At a glance**

- Three-stage architecture with a hard boundary: statistical detection (pandas, deterministic) → LLM diagnosis → executable fix suggestions
- Fully unit-tested with zero API calls — Anthropic client is dependency-injected and mocked in tests
- Production packaging: FastAPI REST service, Docker, CI on every push
- Runs entirely on BigQuery public datasets — clone and run, no proprietary data or GCP billing required
